...
#   glmnet(x=x, y=y, family = "multinomial", alpha = 0, lambda = 0)
# }
...
ordFuncs$fit=function(x, y, first, last, ...){
  tmp=cbind(x, y)
  polr(as.ordered(tmp$y)~., tmp, Hess = TRUE, method = "logistic")
}
ordFuncs$pred=function(object, x){
   #if (!is.data.frame(x)) 
   #  x <- as.data.frame(x, stringsAsFactors = TRUE)
   lvl <- levels(factor(object$lev))
   tmp <- predict(object, x, type="probs")
   tmp <- matrix(tmp, ncol=length(object$lev), byrow = FALSE)
   tmp <- exp(tmp)
   tmp <- t(apply(tmp, 1, function(i) i/sum(i)))
   out <- data.frame(tmp)
   colnames(out) <- lvl
   out$pred <- factor(max.col(tmp), levels = lvl)
   out
}
ordFuncs$rank=function (object, x, y){
...
ctrl=rfeControl(functions = caretFuncs, method = "repeatedcv", index=index, repeats = 1, verbose = TRUE, allowParallel = TRUE)
...
#recursive feature elimination
rfe.mod=rfe(as.ordered(y)~., training.df, sizes=seq(2, dim(training.df)[2]-1, 1) , rfeControl = ctrl, metric = "Accuracy",
            method="polr")
stopCluster(cl)
...
opt.mod=glmnet(x=opt.data, y=y, family = "multinomial", lambda = 0, alpha = 0)
coef.glmnet(opt.mod)
